...
exports.createConstellation = async (req, res) => {
  ...
  exports.updateConstellation = async (req, res) => {
    try {
      const constellation = await Constellation.findOneAndUpdate({ name: req.params.name }, req.body, { new: true });
      res.json(constellation);
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
  ...
  exports.deleteConstellation = async (req, res) => {
    try {
      const constellation = await Constellation.findOneAndDelete({ name: req.params.name });
      res.json(constellation);
    } catch (error) {
      res.status(400).json({ error: error.message });
    }
