...
function validateHashSignature(message2validate) {
  ...
  const json_string = JSON.stringify(
    original_message,
    Object.keys(original_message).sort()
  );
  ...
  if (computed_hash_hex !== received_hash_hex) {
    console.log(
      "Hash del mensaje no coincide. El mensaje puede haber sido alterado."
    );
    return false;
  }
  ...
  try {
    ...
    if (isVerified) {
      ...
      console.log(
        "La firma no es válida. El mensaje puede haber sido alterado o la clave pública es incorrecta."
      );
      return false;
    }
  } catch (e) {
