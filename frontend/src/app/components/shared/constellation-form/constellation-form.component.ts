      ...
      next: (Constellations) => {
        ...
        if (this.num_constellations < 6) {
          const constellation_id =
            this.constellationForm.get('constellation_id')?.value;
          const name = this.constellationForm.get('name')?.value;
          const stations = this.constellationForm.get(
            'constellation_stations'
          )?.value;
          if (stations.length == 3) {
            ...
            if (
              ...
              this.constellationsService
                .getConstellation(this.constellationForm.value.constellation_id)
                .subscribe({
                  next: (data) => {
                  ...
                  error: (error) => {
                    ...
                    this.constellationsService
                      .createConstellations(CONSTELLATION)
                      .subscribe({
                        next: (data) => {
                          this.constellationsService.joinConstellation(CONSTELLATION);
                          location.reload();
                        },
                        error: (error) => {
                          console.log(error);
